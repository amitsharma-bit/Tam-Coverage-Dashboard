...
```
dist/              deployable static site — this is what gets uploaded to AWS
  index.html
  styles.css
...
scripts/              data pipeline — not deployed, regenerates dist/data.js and
                       dist/cslive-data.js from source data
  build_data.py       HubSpot TAM export -> dist/data.js
  build_cs_data.py    CS Live Google Sheet -> dist/cslive-data.js
...
`dist/` has zero build step — it's plain HTML/CSS/JS, safe to upload as-is.
...
python3 verify.py           # sanity-checks the rebuilt dist/data.js
```
...
```bash
cd scripts
python3 nocache_server.py 8000 ../dist
```
...
`dist/` is a static site with no server-side logic, so it deploys as-is to:
- **S3 + CloudFront**: upload the contents of `dist/` to an S3 bucket configured for static
  website hosting (or behind CloudFront), with `index.html` as the index document.
- **Amplify Hosting**: point Amplify at this repo with `dist` as the build output directory
  and no build command.

`data.js` and `cslive-data.js` are static generated files checked into `dist/` — there's no
runtime API call, so no backend/compute is needed. Re-run the scripts above and re-upload
`dist/` whenever the underlying data should refresh.
